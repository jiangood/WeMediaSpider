...
{body}
    ...
    def _on_download_pdf(self):
        ...
        worker = export_article_pdf(title=title, account=account, content=content, parent=self)
        if worker is None:
            return
        self.download_pdf_btn.setEnabled(False)
        self.download_pdf_btn.setText("导出中...")
        worker.pdf_export_success.connect(lambda fp: self._on_pdf_export_done())
        worker.pdf_export_failed.connect(lambda err: self._on_pdf_export_failed(err))

    def _on_pdf_export_done(self):
